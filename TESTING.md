...
| Password reset / resend verification                         | Rewired to `supabase.auth.resetPasswordForEmail` / `.resend()`; both fixed to pass `emailRedirectTo` explicitly — without it, tokens verified server-side but the redirect back into the app could fail or look broken                                                                                                                                                                   |
| Custom email templates                                       | Drafted (`confirm-signup.html`, `reset-password.html` in `ping-me-api/email-templates/`) but not applied — Supabase's dashboard blocks template editing on the default (non-custom-SMTP) email service for free-tier projects as of a June 2026 policy change. Applying them requires custom SMTP, which requires a verified sending domain — deliberately out of scope for this project |
